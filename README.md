...
.favorites-bar.editing .favorite-item .remove-fav {
  display: inline;
}

.favorites-bar.editing .favorite-item:hover .remove-fav {
  color: #a00;
}
...
.favorites-bar.editing ~ .tool-tabs .tool-tab .add-fav {
  display: inline;
}

.tool-tab .add-fav:hover {
  color: #b8860b;
}
...
<div class="container">
  ...
  <div class="favorites-bar" id="favorites-bar">
    ...
    <span class="favorites-list" id="favorites-list"></span>
    <button class="btn-small favorites-edit" id="favorites-edit-btn" onclick="toggleFavoritesEdit()">編集</button>
  </div>
...
function loadFavorites() {
  const saved = localStorage.getItem('texttools-favorites');
  return saved ? JSON.parse(saved) : [];
}

function saveFavorites(favorites) {
  localStorage.setItem('texttools-favorites', JSON.stringify(favorites));
}
...
function renderFavorites() {
  ...
  favorites.forEach((fav, index) => {
    ...
    item.innerHTML = `${fav.name}<span class="remove-fav" onclick="removeFavorite(event, ${index})">×</span>`;
    item.onclick = (e) => {
...
function toggleFavoritesEdit() {
  ...
  const btn = document.getElementById('favorites-edit-btn');
  btn.textContent = bar.classList.contains('editing') ? '完了' : '編集';
  
  // タブにお気に入り追加ボタンを表示/非表示
  document.querySelectorAll('.tool-tab .add-fav').forEach(el => {
    el.style.display = bar.classList.contains('editing') ? 'inline' : 'none';
  });
}

function addFavorite(toolId, toolName) {
  const favorites = loadFavorites();
  if (!favorites.find(f => f.tool === toolId)) {
    favorites.push({ tool: toolId, name: toolName });
...
function initFavorites() {
  ...
  document.querySelectorAll('.tool-tab').forEach(tab => {
    ...
    addBtn.className = 'add-fav';
    addBtn.textContent = '+★';
    addBtn.onclick = (e) => {
      e.stopPropagation();
      addFavorite(tab.dataset.tool, tab.textContent.replace('+★', '').trim());
    };
